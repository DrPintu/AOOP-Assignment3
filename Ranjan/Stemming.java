...
public class Stemming implements ActionListener {
    JFrame f;
    JTextArea t1, t2;
    // JScrollPane sp1, sp2;
    Stemming() {        // Constructor of the class
        f = new JFrame("Stemming Using GUI!");
        f.setSize(800, 1000);

        JButton b = new JButton("Stem the Para");
        b.setBounds(300, 350, 200, 40);
        f.add(b);

        t1 = new JTextArea();
        t1.setBounds(200, 100, 350, 200);
        f.add(t1);

        // sp1 = new JScrollPane(t1);
        // f.add(sp1);
        try {
            FileReader fr = new FileReader("C:\\Documents\\Sem4\\Java\\Projects\\New.txt");
            Scanner scnr = new Scanner(fr);
            while (scnr.hasNextLine()) {
                t1.append(scnr.nextLine());
                t1.append("\n");
            }
        }
    ...
    public void actionPerformed(ActionEvent e) {
        String para = t1.getText();
        String[] words=para.split("\\s");
        t2 = new JTextArea();
        t2.setBounds(200, 400, 350, 200);
        f.add(t2);
        t2.append("Words - Stem word\n\n");
    ...
    public String[] stem(String word) {
        ...
        String[] result = {word, stemmedWord};
        return result;
    }
}
